...
def display_img_enc(net, real_H, dataset):
    ...
    net.eval_mode = True
    net.use_cuda = False
    recon_img = net(torch.matmul(torch.matmul(net.phi.cpu().data, real_H.cpu().data), dataset[i][0])).view(1,-1)
...
def save_model(net, acc, initial_H, name, num_iters, lam, mse):
    torch.save(net.H.data, name + '_Din' + str(net.D_in) + '_Denc' + str(net.D_enc) + '_iters'+ str(num_iters) +'_lam'+ str(lam) +'H.pt')
    torch.save(net.classifier, name + '_Din' + str(net.D_in) + '_Denc' + str(net.D_enc) + '_iters'+ str(num_iters) +'_lam'+ str(lam) +'classifier.pt')
    torch.save(torch.tensor(acc[0]), name + '_Din' + str(net.D_in) + '_Denc' + str(net.D_enc) + '_iters'+ str(num_iters) +'_lam'+ str(lam) +'TrainAcc.pt')
    torch.save(torch.tensor(acc[1]), name + '_Din' + str(net.D_in) + '_Denc' + str(net.D_enc) + '_iters'+ str(num_iters) +'_lam'+ str(lam) +'TestAcc.pt')
    torch.save(torch.tensor(mse), name + '_Din' + str(net.D_in) + '_Denc' + str(net.D_enc) + '_iters'+ str(num_iters) +'_lam'+ str(lam) +'MSE.pt')
    torch.save(initial_H, name + '_Din' + str(net.D_in) + '_Denc' + str(net.D_enc) + '_iters'+ str(num_iters) +'_lam'+ str(lam) +'initial_H.pt')
